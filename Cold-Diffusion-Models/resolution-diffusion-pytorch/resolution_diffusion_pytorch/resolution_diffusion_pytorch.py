...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, batch_size = 16, img=None, t=None):
        ...
        direct_recons = None
        while(t):
            step = torch.full((batch_size,), t - 1, dtype=torch.long).to(self.device_of_kernel)
    ...
    def all_sample(self, batch_size=16, img=None, t=None, times=None):
        ...
        # 3(2), 2(1), 1(0)
        while (times):
            step = torch.full((batch_size,), times - 1, dtype=torch.long).to(self.device_of_kernel)
            x = self.denoise_fn(img, step)
            X_0s.append(x)
            X_ts.append(img)
...
class Trainer(object):
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                data = next(self.dl).to(self.model.device_of_kernel)
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                og_img = next(self.dl).to(self.model.device_of_kernel)
    ...
    def fid_distance_decrease_from_manifold(self, fid_func, start=0, end=1000):
        ...
        for idx in range(len(dataset)):
            img = dataset[idx]
            img = torch.unsqueeze(img, 0).to(self.model.device_of_kernel)
            if idx > start:
        ...
        while(cnt < all_samples.shape[0]):
            ...
            og_x = og_x.to(self.model.device_of_kernel)
            if(self.model.device_of_kernel == 'cuda'):
    ...
    def sample_as_a_mean_blur_torch_gmm_ablation(self, torch_gmm, siz=2, ch=3, clusters=10, sample_at=6, noise=0):
        ...
        for i, img in enumerate(dl, 0):
            print(img.shape)
            img = self.ema_model.module.opt(img.to(self.model.device_of_kernel), t=sample_at)
            img = F.interpolate(img, size=siz, mode='area')
            img = flatten(img).to(self.model.device_of_kernel)
            if all_samples is None:
                all_samples = img
            else:
                all_samples = torch.cat((all_samples, img), dim=0)

        all_samples = all_samples.to(self.model.device_of_kernel)
    ...
    def test_from_data(self, extra_path, s_times=None):
        for batch_idx, x in enumerate(self.dl): 
            og_img = x
            og_img = og_img.to(self.model.device_of_kernel)
            if(self.model.device_of_kernel == 'cuda'):
                og_img = og_img.type(torch.cuda.FloatTensor)
            og_dict = {'og': og_img.cuda()}
            X_0s, X_ts = self.ema_model.all_sample(batch_size=og_img.shape[0], img=og_img, times=s_times)
            ...
            if batch_idx * self.batch_size > 1000:
                break
    ...
    def save_gif(self, X_0s, X_ts, extra_path, init_recon=None, og=None):
        ...
        for i in range(len(X_0s)):
            ...
            if init_recon is not None:
                ...
                og = og.to('cpu')
                all_images_grid = all_images_grid.to('cpu')
