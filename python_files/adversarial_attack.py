...
class AdversarialAttack:
    ...
    def adversary_generator(self, batch_size):
        while True:
            images = []
            labels = []

            for batch in range(batch_size):
                N = torch.randint(0, len(self.test_loader.dataset), (1,))
                data = self.test_loader.dataset[N]
                image, label = data[0].unsqueeze(0), torch.tensor(data[1])

                perturbations = self.generate_adversary(image, label)
